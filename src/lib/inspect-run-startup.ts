...
export const inspectRunStartup = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await convex.mutation(api.runtime.updateRunQueueState, {
      ...
      body:
        executionState.browserProvider === "local_chrome"
          ? "The local run was created, but no healthy local helper has claimed it yet. Make sure Bun is installed, start `bun run local-helper`, keep Chrome open, and approve the Chrome debugging permission prompt."
