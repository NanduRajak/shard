...
async function runAgentLoop({
  ...
  for (let stepIndex = 1; stepIndex <= config.maxAgentSteps; stepIndex += 1) {
    ...
    const toolResults =
      result?.steps.flatMap((step: any) => step.toolResults ?? []) ?? [];
...
async function runSnapshotStage({
  ...
  const pageReview = await generateObject({
    ...
    if (isQaOperationAborted(runtime, error)) {
      throw buildQaAbortError(
        "QA run stopped during page review",
        snapshot.url,
      );
    }
...
async function throwIfStopRequested({
  ...
  if (bufferedFindings && findingSignatures && bufferedFindings.length > 0) {
    await flushBufferedFindings({
      bufferedFindings,
      findingSignatures,
      pageCandidates,
  ...
  throw new QaRunCancelledError(
    currentStep,
    pageUrl ?? executionState.currentUrl ?? undefined,
  );
}
...
async function verifyTaskCompletion({
  ...
  const assessment = await generateObject({
    ...
    if (isQaOperationAborted(runtime, error)) {
      throw buildQaAbortError(
        "QA run stopped during task verification",
        snapshot.url,
      );
    }
...
async function runRetriedAction<T>(operation: () => Promise<T>) {
  ...
  throw lastError instanceof Error
    ? lastError
    : new Error("Unknown tool execution error");
}
